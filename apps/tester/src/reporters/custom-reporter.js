const RPClient = require('./mock-client');
const getOptions = require('@reportportal/agent-js-jest/utils/getOptions');
const {
  getClientInitObject,
  getSuiteStartObject,
...
class CustomReporter {
  ...
  _startTest(test, testPath) {
    ...
    const parentId =
      this.tempTestIds.get(test.ancestorTitles.slice(0, -1).join('/')) ||
      tempSuiteId;
    const { tempId, promise } = this.client.startTestItem(
      testStartObj,
      this.tempLaunchId,
  ...
  _startStep(test, isRetried, testPath) {
    ...
    const parentId =
      this.tempTestIds.get(test.ancestorTitles.join('/')) || tempSuiteId;
    const { tempId, promise } = this.client.startTestItem(
      stepStartObj,
      this.tempLaunchId,
      parentId
  ...
  _finishSkippedStep(isRetried) {
    ...
    const issue =
      this.reportOptions.skippedIssue === false
        ? { issueType: 'NOT_ISSUE' }
        : null;
